...
Before compositing a supplied closed-eye portrait, inspect each detached eyelid PNG alone on both dark and light backgrounds at native scale and enlarged. An eye-sized crop may still contain ear, brow, cheek or a low-alpha skin rectangle; its alpha footprint must follow only the painted lid region, with feathered edges. Do not over-threshold the skin by color until eyelashes or soft lid shading break apart. Then compare the feature-free base, open, half-closed and fully closed frames on the assembled face, including a paused and moving preview. Record whether the new character follows the reference character's **control semantics, pulse timing, open-eye contraction, and open/closed layer blend** separately; matching toggle labels alone is not parity. The current Eris and Miffy viewers both use quadratic closure (`blink²`), contract/fade the open artwork and fade in the closed lid; Eris deforms separate eye layers in WebGL, while Miffy contracts a registered canvas eye stage. Never describe either as merely pasting a closed lid, or claim that shared controls imply pixel-identical deformation. Keep character-specific eye geometry and source pixels even when reusing a proven blend rule.

Also inspect the *base, open and eye-white backings*, not just the closed lid: a rectangular backing can extend into an ear or hair even when its closed-lid alpha is clean. Compare each backing composite against the pinned source head outside the painted eye. If the eye art ends before that collision, `scripts/trim_eye_backing_edge.mjs` can create a new alpha-only candidate from the previous manifest; first measure the safe boundary and verify iris, lash, lid and mask have no pixels there. Its right-edge arguments mean feather-start/zero-from; its left-edge arguments mean zero-through/feather-end. A narrow lower ear-root box may be supplied only with a fixed underlay that matches every newly removed neutral pixel exactly. By default preserve the original neutral `open` artwork and use the optional eye-only `openMotion` inside the moving blink stage. If the user requires strict part ownership, pass that pixel-identical fixed-head underlay to trim neutral `open` as well, then prove the browser-rendered zero-blink frame remains identical. Keep the previous manifest for rollback, compare ear-root pixels at open/half/closed states, and inspect independent head/hair movement. Do not trim a genuinely painted feature or hide missing anatomy with a skin plate.
