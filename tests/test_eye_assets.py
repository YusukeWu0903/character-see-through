...
def test_derives_full_canvas_left_right_assets_and_safe_gaze_limit(tmp_path: Path):
    ...
    layer(boxes=((7, 17, 24, 33), (47, 17, 64, 33))).save(tmp_path / "eyelash.png")
    ...
    assert report["eyeCenters"]["left"] != report["eyeCenters"]["right"]
    for name in ("irides_left", "irides_right", "eyewhite_left", "eyewhite_right", "eyelash_left", "eyelash_right"):
...
def test_rejects_missing_eye_component(tmp_path: Path):
    ...
    layer().save(tmp_path / "eyelash.png")
    try:
        derive(tmp_path)
...
def test_rederiving_eye_geometry_preserves_approved_eyelid_manifest(tmp_path: Path):
    layer().save(tmp_path / "irides.png")
    layer(boxes=((8, 18, 23, 32), (48, 18, 63, 32))).save(tmp_path / "eyewhite.png")
    layer(boxes=((7, 17, 24, 33), (47, 17, 64, 33))).save(tmp_path / "eyelash.png")
    first = derive(tmp_path)
    first["closedEyelids"] = {"schemaVersion": 3, "visualReview": {"status": "passed"}}
    manifest = tmp_path / "_rig_assets" / "eye_assets.json"
    import json
    manifest.write_text(json.dumps(first), encoding="utf-8")
    second = derive(tmp_path)
    assert second["closedEyelids"] == first["closedEyelids"]
