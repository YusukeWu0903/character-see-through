...
`/preview-deform` 已實作 Eris 專用的上身側彎、頭部側傾、頸部及腰部共享網格過渡。
第二階段測試通過：中性姿勢柔性與剛性像素對齊、重疊圖層使用同一位置場、
腿不受上身局部側彎帶動、錨點／上限設定保存及匯入匯出往返、非法設定拒絕。
Chromium 實測載入 32 張圖層、沒有執行錯誤，30 幀約 491ms。

使用者回饋驅動本階段：第一階段「還行，但可動部位少、辨識度不高」。
本階段增加可見但受限制的上身與頭部動作；視覺接受度仍應由使用者在放大視角檢查頸、肩、腰接縫。
