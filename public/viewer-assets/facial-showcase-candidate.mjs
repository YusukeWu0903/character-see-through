export const MOUTH_OPTIONS=[
  ...
  ['angry','抿嘴 · 無髮憤怒原圖'],['slight','微開 · 原圖 2'],
  ['wide','大開口 · 原圖 4'],['round','小圓口 · 原圖 5'],
  ['teeth','齒唇音 · 原圖 7'],
];
export const EMOTION_OPTIONS=[
  ['neutral','原始表情'],['happy','開心／微笑 · 無髮原圖'],
  ['angry','生氣／憤怒 · 無髮原圖'],
];
...
export async function createFacialCandidate(layers,renderer,assetPrefix){
  ...
  if(!mouth||!brows||!seam)throw new Error('整合表情候選需要 mouth、eyebrow 與 seam_repair_head 圖層');
  const originalBrows=brows.image;
  const [mouthImages,mouthlessSeam,happyBrows,angryBrows]=await Promise.all([
    ...
    loadImage(assetPrefix+'eyebrow_happy.png'),
  ...
  return {
    update(mode,emotion){
      if(!images.has(mode)||!EMOTION_OPTIONS.some(([name])=>name===emotion))throw new Error('未知的口型或表情');
      if(mode!==activeMouth){mouth.image=images.get(mode);activeMouth=mode;}
      if(emotion!==activeEmotion){
        brows.image=emotion==='happy'?happyBrows:emotion==='angry'?angryBrows:originalBrows;
