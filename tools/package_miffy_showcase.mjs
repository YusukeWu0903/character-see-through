...
const rigPath = '_review/motion_v50/rig.json';
...
async function main() {
  ...
  if (newest.task !== task || newest.candidate !== 'motion_v50' ||
  ...
  const mouth=JSON.parse(await copyAsset(resolved.mouthRig.manifest,resolved.mouthRig.manifestSha256));
  const mouthDir=path.posix.dirname(resolved.mouthRig.manifest);
  await copyAsset(mouth.targetHead,mouth.targetHeadSha256);
  for(const spec of [mouth.backing,...Object.values(mouth.mouths)])await copyAsset(`${mouthDir}/${spec.asset}`,spec.sha256);
  ...
  const modules = ['assembly-motion', 'rig', 'grounded-sway', 'stance-field',
    ...
    'hair-idle-state', 'pitch-follow-field', 'arm-sway-field', 'mouth-transplant', 'shared-field-adapter', 'mesh-renderer'];
  ...
  for (const name of modules) {
    ...
    if (name === 'assembly-motion') {
      const substitutions = [
        ...
        ["$('candidate-title').textContent='Miffy 全身動態 '+rig.candidate+' · 非正式審查候選';", "$('candidate-title').textContent='Miffy · v50 階段展示';"],
        ["$('status').textContent='已載入 17 層 · '+rig.candidate+' 待審';", "$('status').textContent='已載入 17 層 · v50 階段展示';"]
      ];
  ...
  const htmlSubstitutions = [
    ...
    ['<strong id="candidate-title">Miffy 全身動態 · 非正式審查候選</strong> <small>本機預覽，不改正式圖層與設定</small><a id="latest" hidden>開啟新版動態</a>', '<strong id="candidate-title">Miffy · v50 階段展示</strong> <small>共用 GPU 動態 · 六種口形</small><a id="latest" hidden></a>'],
    ['<script type="module" src="./assembly-motion.mjs?review-runtime=v49-full-gpu"></script>', '<script type="module">\nconst demoPath=location.pathname.replace(/\\/index\\.html$/,\'\').replace(/\\/$/,\'\');\nawait import(demoPath+\'/viewer-assets/assembly-motion.mjs?v=50\');\n</script>']
  ];
  ...
  await writeFile(path.join(output,'release.json'),JSON.stringify({task,candidate:newest.candidate,
    approval:'User accepted v50 motion/performance and requested publication on 2026-09-26',
    stageCheckpoint:true,nativeTextureSize:1280,remaining:['one final Canvas copy','audio lip-sync not implemented'],
    assets:[...copied].sort()},null,2));
