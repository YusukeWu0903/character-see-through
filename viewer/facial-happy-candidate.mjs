...
export const EMOTION_OPTIONS=[['neutral','原始表情'],['happy','開心／微笑 · 無髮原圖']];
...
export async function createFacialCandidate(layers,renderer,assetPrefix){
  ...
  if(!mouth||!brows||!seam)throw new Error('開心表情候選需要 mouth、eyebrow 與 seam_repair_head 圖層');
  const originalBrows=brows.image;
  const [mouthImages,mouthlessSeam,happyBrows]=await Promise.all([
  ...
  return {
    update(mode,emotion){
      if(!images.has(mode)||!EMOTION_OPTIONS.some(([name])=>name===emotion))throw new Error('未知的口型或開心表情');
      if(mode!==activeMouth){mouth.image=images.get(mode);activeMouth=mode;}
      if(emotion!==activeEmotion){
        brows.image=emotion==='happy'?happyBrows:originalBrows;
